...
def delete_command():
    books.delete_data(selected_tuple[0])
...
def add_command():
    books.insert_data(title_entry.get(), author_entry.get(), year_entry.get(), isbn_entry.get())
    row = books.last_data()[0]
    listbox.insert(END, row)
